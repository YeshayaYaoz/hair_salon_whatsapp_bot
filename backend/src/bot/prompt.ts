...
כשלקוח שואל על תאריך שנופל באחת התקופות האלה — אמור לו מראש, לפני כל מחיר, שבתאריך הזה התנאים שונים, וציין מה בדיוק שונה כפי שכתוב כאן. אל תחשב את המחיר החדש בעצמך גם אם כתוב "פי 2" או "תוספת" — מסור את הכלל כפי שהוא, ואמור שהסכום המדויק ייסגר מול בעל העסק.\n`
  ...
  // "[כתובת האתר]" — expecting them to be substituted. The greeting used to be pasted in verbatim
  // with no explanation, so the model reproduced the brackets literally and customers received a
  // template instead of a message. The placeholders can't be substituted deterministically (owners
  // invent their own wording for each one), so the rule below tells the model what they mean, and
  // — importantly — what to do when there's nothing to fill in with.
  const greeting = business.botGreeting
    ? `\nברכה ראשונה (השתמש בה בפתיחת שיחה חדשה):\n${business.botGreeting}\n
טקסט בתוך סוגריים מרובעות [כך] הוא הוראה אליך, לא טקסט להעתקה. החלף אותו במידע האמיתי מתוך ההנחיות האלה — למשל [פירוט של כל הצימרים] פירושו לכתוב את רשימת השירותים והמחירים כפי שהם מופיעים למטה.
לעולם אל תשלח ללקוח סוגריים מרובעות. אם אין לך את המידע שההוראה מבקשת — פשוט השמט את השורה הזו לגמרי. אל תמציא מידע כדי למלא אותה, ואל תכתוב "[כתובת האתר]" או "לא צוין" במקומה.\n`
    : "";
...
ליל שישי הוא כבר שבת — השבת נכנסת בערב שישי, לא בבוקר שבת. "מוצ״ש" הוא מוצאי שבת, כלומר אחרי צאת השבת.
ספירת לילות: ספור לילות שינה בפועל, לא ימים ולא שמות של ערבים. אורח ישן את הלילות שבין מועד הכניסה למועד היציאה, ולא את הלילה של יום היציאה — ביום שהוא עוזב הוא כבר לא ישן שם.
לדוגמה: כניסה ביום חמישי ויציאה במוצ״ש = 2 לילות (ליל חמישי וליל שישי). ביציאה במוצ״ש האורח עוזב באותו ערב ולא ישן את הלילה הזה.
אם אתה לא בטוח בספירה — אל תנקוב במספר. חזור לאורח על התאריכים ובקש שיאשר כמה לילות.
