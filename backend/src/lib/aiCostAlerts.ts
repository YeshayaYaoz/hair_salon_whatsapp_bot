 ...
 * a premium month's revenue, under 2%), so crossing this means something changed, not that the
...
export const AI_BUDGET_AGOROT_BY_PLAN: Record<string, number> = {
  ...
  premium: Math.round(374.9 * 100 * AI_BUDGET_SHARE_OF_REVENUE), // ₪93.73
  ultra: Math.round(749.9 * 100 * AI_BUDGET_SHARE_OF_REVENUE), // ₪187.48
};
