...
const RECOMMENDED: RequiredVar[] = [
  ...
  { name: "TORI_WABA_ID", description: "Tori's own WABA — the automatic WhatsApp setup puts a provisioned number on it; without it every number order ends in the 'auto-setup failed' email and a manual workflow run" },
  { name: "META_SYSTEM_USER_TOKEN", description: "system-user token for Tori's own Meta business — drives the automatic WhatsApp setup (add/verify/register) for provisioned numbers" },
