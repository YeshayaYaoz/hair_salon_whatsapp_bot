 ...
 * so this catches the case no genuine booking conversation reaches: a hundred messages in a day.
 */
describe("checkDailyCap", () => {
  ...
  it("allows exactly the cap, and stops the one after it", async () => {
    // The count excludes the message being handled, so the 100th still gets answered.
  ...
  it("counts only this customer's own messages, in a rolling 24h window", async () => {
    count.mockResolvedValue(0);
    await checkDailyCap("biz1", "972500000000");

    const where = count.mock.calls[0][0].where;
    expect(where).toMatchObject({ businessId: "biz1", phone: "972500000000", role: "user" });
