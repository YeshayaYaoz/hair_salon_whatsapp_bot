...
async def pre_call_handler(call_request: CallRequest) -> Optional[PreCallResult]:
    ...
    # Model pinned, not defaulted. Two reasons, one urgent: an unpinned voice model given a line
    # that is mostly Latin letters and digits — exactly what an email read-back is — can drift into
    # another language entirely, and a live confirmation turn came out as Chinese. And Cartesia is
    # sunsetting sonic-3 and sonic-turbo on 2026-10-20, so whatever the default resolves to today
    # may simply stop existing. sonic-3.5 is their current stable.
    tts: Dict[str, Any] = {"language": LANGUAGE, "model": os.environ.get("TORI_TTS_MODEL", "sonic-3.5")}
...
FORMS = {
    "feminine": {
        ...
        "no_media": 'כשמבקשים תמונות או פרטים — השתמשי ב-send_details ושלחי לוואטסאפ של המתקשר. אל תבקשי כתובת מייל: המספר שלו כבר ידוע והוואטסאפ הוא ברירת המחדל. רק אם הוא אומר שאין לו וואטסאפ, או מבקש מייל במפורש — בקשי כתובת מייל, חזרי עליה ובקשי אישור, ואז שלחי במייל. אל תגידי "נשלח" לפני שהכלי החזיר שהשליחה הצליחה. וחשוב: לפני שאת קוראת לכלי — אמרי משפט קצר כמו "רגע, שולחת לך עכשיו", וגם לפני שליחה שנייה באותה שיחה (מייל אחרי וואטסאפ) — בכל פעם מחדש. כלי לוקח כמה שניות, ובשיחת טלפון השניות האלה נשמעות כמו קו שנפל, לא כמו מישהי שעובדת.',
        "quote_with_rules": 'בכל פעם שאת מוסרת מחיר — אמרי גם את ההחרגות שלמעלה באותה נשימה, גם אם לא שאלו. מחיר בלי ההחרגות הוא מה שהלקוח יזמין לפיו.',
        "spoken_email": 'כתובת מייל בשיחה עוברת בשני כיוונים, ואל תבלבלי ביניהם. כששומעים אותה: "שטרודל" זה @, "נקודה" זה ., "מקף" זה -, "קו תחתון" זה _, "ג\'ימייל" זה gmail.com, "וואלה" זה walla.co.il, "הוטמייל" זה hotmail.com — ול-send_details תעבירי את הכתובת הכתובה, למשל yeshaya@gmail.com. כשאת חוזרת עליה בקול לאישור — משפט שלם עם תגית איות: כתבי "אני חוזרת על הכתובת: <spell>Y28112000</spell> שטרודל ג\'ימייל נקודה קום. נכון?" — מה שבתוך <spell> יוקרא אוטומטית תו-תו, ספרה-ספרה. עטפי ב-<spell> רק את החלק שלפני השטרודל; את הסימן @ אל תכניסי לתגית ואל תכתבי בכלל — אמרי "שטרודל" כמילה, ואת הדומיין במילים ("ג\'ימייל נקודה קום"). לעולם אל תקריאי רשימת מילים מנותקות עם פסיקים — זה גורם לקול לצאת ג\'יבריש. מספרים שנאמרים במילים המירי לספרות: "אלפיים" זה 2000, "חמישים" זה 50, "עשרים ושמונה" זה 28, "אחת עשרה" זה 11, "חמש מאות" זה 500 — ו"אחת אחת" זה שתי ספרות: 11. אם לא הצלחת להבין — בקשי שיאיית אות-אות, או הציעי לשלוח לוואטסאפ.',
    ...
    "masculine": {
        ...
        "no_media": 'כשמבקשים תמונות או פרטים — השתמש ב-send_details ושלח לוואטסאפ של המתקשר. אל תבקש כתובת מייל: המספר שלו כבר ידוע והוואטסאפ הוא ברירת המחדל. רק אם הוא אומר שאין לו וואטסאפ, או מבקש מייל במפורש — בקש כתובת מייל, חזור עליה ובקש אישור, ואז שלח במייל. אל תגיד "נשלח" לפני שהכלי החזיר שהשליחה הצליחה. וחשוב: לפני שאתה קורא לכלי — אמור משפט קצר כמו "רגע, שולח לך עכשיו", וגם לפני שליחה שנייה באותה שיחה (מייל אחרי וואטסאפ) — בכל פעם מחדש. כלי לוקח כמה שניות, ובשיחת טלפון השניות האלה נשמעות כמו קו שנפל, לא כמו מישהו שעובד.',
        "quote_with_rules": 'בכל פעם שאתה מוסר מחיר — אמור גם את ההחרגות שלמעלה באותה נשימה, גם אם לא שאלו. מחיר בלי ההחרגות הוא מה שהלקוח יזמין לפיו.',
        "spoken_email": 'כתובת מייל בשיחה עוברת בשני כיוונים, ואל תבלבל ביניהם. כששומעים אותה: "שטרודל" זה @, "נקודה" זה ., "מקף" זה -, "קו תחתון" זה _, "ג\'ימייל" זה gmail.com, "וואלה" זה walla.co.il, "הוטמייל" זה hotmail.com — ול-send_details תעביר את הכתובת הכתובה, למשל yeshaya@gmail.com. כשאתה חוזר עליה בקול לאישור — משפט שלם עם תגית איות: כתוב "אני חוזר על הכתובת: <spell>Y28112000</spell> שטרודל ג\'ימייל נקודה קום. נכון?" — מה שבתוך <spell> יוקרא אוטומטית תו-תו, ספרה-ספרה. עטוף ב-<spell> רק את החלק שלפני השטרודל; את הסימן @ אל תכניס לתגית ואל תכתוב בכלל — אמור "שטרודל" כמילה, ואת הדומיין במילים ("ג\'ימייל נקודה קום"). לעולם אל תקריא רשימת מילים מנותקות עם פסיקים — זה גורם לקול לצאת ג\'יבריש. מספרים שנאמרים במילים המר לספרות: "אלפיים" זה 2000, "חמישים" זה 50, "עשרים ושמונה" זה 28, "אחת עשרה" זה 11, "חמש מאות" זה 500 — ו"אחת אחת" זה שתי ספרות: 11. אם לא הצלחת להבין — בקש שיאיית אות-אות, או הצע לשלוח לוואטסאפ.',
...
def build_prompt(ctx: Dict[str, Any], caller_known: bool = True) -> str:
    ...
    if ctx.get("pricingNotes"):
        # The rule, not just the text. A price said out loud without its exclusions is the one a
        # caller books on, and the owner is the one who has to correct it at check-in — so the
        # instruction is tied to the act of quoting rather than left to the model's judgement of
        # when an exclusion is "relevant".
        parts += ["", "## כללי תמחור", str(ctx["pricingNotes"]), f["quote_with_rules"], f["as_written"]]
...
def build_agent(resolved: Dict[str, Any], called: str, caller_num: str) -> LlmAgent:
    ...
    async def send_details(
        ...
        if channel == "email" and not email_confirmed:
            return (
                "עצור — עוד לא שלחתי. קודם חזור על הכתובת במשפט שלם עם תגית איות — "
                '"אני חוזר: <spell>החלק שלפני השטרודל</spell> שטרודל ג\'ימייל נקודה קום. נכון?" — '
                "חכה שהמתקשר יגיד שהיא נכונה, ורק אז קרא שוב עם email_confirmed=true."
            )
