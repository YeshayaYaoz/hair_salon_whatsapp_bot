...
FORMS = {
    "feminine": {
        ...
        "spoken_email": 'כתובת מייל בשיחה עוברת בשני כיוונים, ואל תבלבלי ביניהם. כששומעים אותה: "שטרודל" זה @, "נקודה" זה ., "מקף" זה -, "קו תחתון" זה _, "ג\'ימייל" זה gmail.com, "וואלה" זה walla.co.il, "הוטמייל" זה hotmail.com — ול-send_details תעבירי את הכתובת הכתובה, למשל yeshaya@gmail.com. כשאת חוזרת עליה בקול לאישור — תו-תו, כל תו בנפרד: אותיות לטיניות בשמן, כל ספרה כמילה עברית אחת ("שתיים, שמונה, אחת, אחת, שתיים, אפס, אפס, אפס"), והסימנים בשמם העברי ("שטרודל", "נקודה"). לעולם אל תרכיבי מספרים בהקראה — לא "עשרים ושמונה", לא "אלפיים", לא "אחת עשרה": מי שמאשר כתובת צריך לשמוע כל ספרה לבד. ולעולם אל תאמרי "at" ואל תקריאי את הסימן @ עצמו. מספרים שנאמרים במילים המירי לספרות: "אלפיים" זה 2000, "חמישים" זה 50, "עשרים ושמונה" זה 28, "אחת עשרה" זה 11, "חמש מאות" זה 500 — ו"אחת אחת" זה שתי ספרות: 11. אם לא הצלחת להבין — בקשי שיאיית אות-אות, או הציעי לשלוח לוואטסאפ.',
    ...
    "masculine": {
        ...
        "spoken_email": 'כתובת מייל בשיחה עוברת בשני כיוונים, ואל תבלבל ביניהם. כששומעים אותה: "שטרודל" זה @, "נקודה" זה ., "מקף" זה -, "קו תחתון" זה _, "ג\'ימייל" זה gmail.com, "וואלה" זה walla.co.il, "הוטמייל" זה hotmail.com — ול-send_details תעביר את הכתובת הכתובה, למשל yeshaya@gmail.com. כשאתה חוזר עליה בקול לאישור — תו-תו, כל תו בנפרד: אותיות לטיניות בשמן, כל ספרה כמילה עברית אחת ("שתיים, שמונה, אחת, אחת, שתיים, אפס, אפס, אפס"), והסימנים בשמם העברי ("שטרודל", "נקודה"). לעולם אל תרכיב מספרים בהקראה — לא "עשרים ושמונה", לא "אלפיים", לא "אחת עשרה": מי שמאשר כתובת צריך לשמוע כל ספרה לבד. ולעולם אל תאמר "at" ואל תקריא את הסימן @ עצמו. מספרים שנאמרים במילים המר לספרות: "אלפיים" זה 2000, "חמישים" זה 50, "עשרים ושמונה" זה 28, "אחת עשרה" זה 11, "חמש מאות" זה 500 — ו"אחת אחת" זה שתי ספרות: 11. אם לא הצלחת להבין — בקש שיאיית אות-אות, או הצע לשלוח לוואטסאפ.',
...
def build_agent(resolved: Dict[str, Any], called: str, caller_num: str) -> LlmAgent:
    ...
    async def send_details(
        ...
        if channel == "email" and not email_confirmed:
            return (
                "עצור — עוד לא שלחתי. קודם חזור על הכתובת בקול תו-תו: כל ספרה כמילה עברית נפרדת "
                '("שתיים, שמונה, אחת, אחת"), אותיות בשמן, "שטרודל" בשביל @ — בלי להרכיב מספרים. '
